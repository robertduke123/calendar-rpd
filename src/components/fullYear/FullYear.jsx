	...
	return (
		<div className="full-year">
			<div className="calendar-head flex-row-cent">
				<h3>{currentYear}</h3>
				<div className="dir-btn" onClick={prevYear}>
					&#10094;
				</div>
				<div className="dir-btn" onClick={nextYear}>
					&#10095;
				</div>
			</div>
				...
				{days.map((day, indx) => {
					return isBefore(day, today) ? (
						<div
							key={"full" + indx}
							className="full-unit"
							style={{ backgroundColor: "rgb(164, 237, 199)" }}></div>
					) : format(today, "dd MMM yyyy") === format(day, "dd MMM yyyy") ? (
						<div
							key={"full" + indx}
							className="full-unit hov"
							style={{ cursor: "pointer", backgroundColor: "red" }}
							onClick={() => dispatch(setSelectedDay(day))}></div>
					) : items
							?.map((item) =>
								item.dates?.includes(format(day, "E MMM dd yyyy"))
							)
							.includes(true) ||
					  items.map((item) => item[format(day, "E")]).includes(true) ? (
						<div
							key={"full" + indx}
							className="full-unit hov"
